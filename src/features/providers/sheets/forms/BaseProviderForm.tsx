...
/** Module-level constant to avoid creating a new array reference for the picker on each render. */
...
export function BaseProviderForm({
  ...
  /**
   * Candidate catalog = discovered models ∪ model names already configured in the form.
   *
   * Both sources may be empty: Vertex supports exclusions but is not in MODEL_DISCOVERY_BRANDS,
   * and other brands have no discovered models until the user runs discovery. An empty catalog
   * is therefore normal, and the picker must fall back to a rule-only editor.
   */
        ...
        <Collapsible label={t('providersPage.form.excludedSection')}>
          <div className={styles.field}>
            <ExcludedModelsPicker
              ...
              disabled={mutating}
              // `'*'` means this provider is disabled and is owned only by the Disabled toggle below.
              // The picker filters it in both directions and explains why manually entered `*` is rejected.
