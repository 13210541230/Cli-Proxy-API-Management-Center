...
import { providersWorkbenchApi as providersApi } from '@/services/api';
...
/* -------------------------------------------------------------------------- */
/* Form-to-backend configuration conversion.                                   */
/* -------------------------------------------------------------------------- */
 ...
 * `stripDisableAllModelsRule` maps it to that flag on load, and saving restores it from the flag.
 * Filter manually entered `'*'` here so the excluded-model editor cannot enable or disable a provider.
 * Exported so tests/providerExcludedModelsDisableRule.test.ts can enforce this invariant.
 */
