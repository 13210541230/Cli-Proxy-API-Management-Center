...
/** Maximum derived chips to render; beyond this, show only the count to avoid overwhelming the field. */
const DERIVED_CHIP_LIMIT = 8;

export interface ExcludedModelsPickerProps {
  /** Normalized rules; callers may store text or a Set and adapt once at the boundary. */
  ...
  /** Actual disabled state (disconnected or saving). Never disable only because the catalog is empty. */
  disabled?: boolean;

  /** Rules the picker must neither read nor write; provider forms pass `['*']`. */
...
export function ExcludedModelsPicker({
  ...
  /**
   * Reserved rules are stripped at the boundary, so the picker never sees or writes them back.
   * The provider form's `'*'` (disabled) is owned exclusively by its disabled toggle.
   */
  const rules = useMemo(
    () =>
  ...
  const commit = useCallback(
    (next: readonly string[]) => {
      // Filter reserved rules again at the boundary so manually entered `*` cannot escape the editor.
  ...
  /** Models derived from wildcard matches, excluding explicit selections shown as solid chips. */
  ...
  /** Clear exact selections only; wildcard rules have their own editor and must not be erased here. */
  ...
  const summaryText = () => {
    ...
    // Without a catalog, report the number of rules rather than guessing how many models remain available.
