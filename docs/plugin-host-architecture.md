...
- Plugin resources are isolated in an iframe and loaded only from a plugin-declared resource path or an explicitly absolute resource URL. The host appends the parent origin as the `cpa_plugin_host_origin` query parameter; the bridge verifies both the iframe window and its exact declared resource origin, then rejects paths outside the current plugin namespace and unsupported methods. Plugins should use that parameter (falling back to `document.referrer`) as the `postMessage` target; the host never uses wildcard origins for responses.
